# River Watch AI 推理架构统一增量包 20260714
...
- `group@structure` 未在 600 秒内完成两个子进程及 MIGraphX 模型加载时自动回滚。
...
```powershell
scp E:\river-video-app-V3.0\river-watch-ai-runtime-topology-increment-20260714.zip `
    E:\river-video-app-V3.0\river-watch-ai-runtime-topology-increment-20260714.zip.sha256 `
...
set -e
cd /home/ai-river
sha256sum -c river-watch-ai-runtime-topology-increment-20260714.zip.sha256
rm -rf river-watch-ai-runtime-topology-increment-20260714
unzip -o river-watch-ai-runtime-topology-increment-20260714.zip
cd river-watch-ai-runtime-topology-increment-20260714
sha256sum -c SHA256SUMS
bash -n scripts/*.sh
chmod +x scripts/*.sh
sudo -v

LOG="/home/ai-river/ai-runtime-group-standard-apply-$(date +%Y%m%d-%H%M%S).log"
nohup sudo -n env \
...
模型首次加载可能持续数分钟。成功日志必须出现 `DONE` 和 `VERIFY OK`。

## 复核

```bash
cd /home/ai-river/river-watch-ai-runtime-topology-increment-20260714
...
sudo bash scripts/rollback-ai-runtime-topology-20260714.sh \
  /home/ai-river/river-watch/backups/ai-runtime-group-standard-20260714-实际时间戳
```
