...
def test_frontend_validates_alarm_and_camera_before_displaying_evidence():
    ...
    assert "if (!metadataResponse.ok" in source
    assert "if (!evidenceMatchesSelectedAlarm(metadata, alarmId, expectedCameraId))" in source
    assert "证据与当前告警或摄像机不匹配，已拒绝显示" in source
    assert "if (!capturedAlarmEvidenceMetadata.value || !capturedAlarmEvidenceUrl.value) return undefined" in source
    assert "const boxes = Array.isArray(persistedBoxes) ? persistedBoxes : []" in source
    assert "{ url: alarm.snapshot" not in source
