...
TS="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="$APP_DIR/backups/alarm-evidence-annotations-layout-20260715-$TS"
SUDO=""
[ "$(id -u)" -ne 0 ] && SUDO="sudo"
BACKEND_CHANGED=0
FRONTEND_CHANGED=0
...
echo "== River Watch historical annotation and 3-column review increment =="
...
check_container_hash server.mjs 88ebc5a208c1c3e641e167ddffd59876c5371118ae0f21d04bca3e92db238ba3
check_container_hash store.mjs 7be18cc8590578aa9dc65fb6f204e05fc8683c035e28e6993f5adc825ed8ddab
...
mapfile -t old_backups < <(find "$APP_DIR/backups" -maxdepth 1 -mindepth 1 -type d -name 'alarm-evidence-annotations-layout-20260715-*' -printf '%T@ %p\n' | sort -rn | awk 'NR>2 {$1=""; sub(/^ /, ""); print}')
