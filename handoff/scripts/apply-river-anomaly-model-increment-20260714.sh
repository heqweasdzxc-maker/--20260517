...
BACKUP_DIR="$APP_DIR/backups/river-anomaly-model-20260714-$STAMP"
RECEIPT="$APP_DIR/logs/ops/river-anomaly-model-20260714-$STAMP.md"
MUTATED=0
DB_UPDATED=0
...
wait_group_model() {
  local group="$1"
  local since="$2"
  local unit="river-ai-group@$group.service"
  local i logs
    ...
    if systemctl is-active --quiet "$unit"; then
      ...
      if grep -Fq "$MODEL_NAME" <<<"$logs"; then
        ...
        if ! grep -Fq 'MIGraphXExecutionProvider' <<<"$logs"; then
          log "$group loaded the model but GPU provider was not confirmed"
          printf '%s\n' "$logs" | tail -120
          return 1
        fi
        log "$group active with $MODEL_NAME on MIGraphXExecutionProvider"
        return 0
      fi
    fi
    sleep 2
...
systemctl is-active --quiet river-ai-group@structure.service || fail "structure is not active before deployment"
if systemctl is-active --quiet river-ai-batch@river.service; then
  fail "parallel river-ai-batch@river is active; audit duplicate inference before model deployment"
fi
...
log "6. Canary activate river-a only"
CANARY_SINCE="$(date --iso-8601=seconds)"
systemctl restart river-ai-group@river-a.service
wait_group_model river-a "$CANARY_SINCE" || fail "river-a canary failed"

log "7. Activate river-b"
RIVER_B_SINCE="$(date --iso-8601=seconds)"
systemctl restart river-ai-group@river-b.service
wait_group_model river-b "$RIVER_B_SINCE" || fail "river-b activation failed"

log "8. Update camera runtime metadata after both groups are healthy"
...

{
  ...
  echo "- Labels: $LABELS"
  echo "- Scope: CH01-CH08 / river-a + river-b"
  echo "- Structure group: unchanged"
...
log "10. Keep only the latest three backups for this model increment"
mapfile -t OLD_BACKUPS < <(
  find "$APP_DIR/backups" -mindepth 1 -maxdepth 1 -type d \
    -name 'river-anomaly-model-20260714-*' -printf '%p\n' | sort -r | tail -n +4
)
for old in "${OLD_BACKUPS[@]:-}"; do
  case "$old" in
    "$APP_DIR"/backups/river-anomaly-model-20260714-*) rm -rf -- "$old" ;;
  esac
done
