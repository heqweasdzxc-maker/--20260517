# River Watch AI 推理架构统一增量包 20260715 v2
...
v2 修正了首版只接受河道 group 已运行基线的问题。它兼容当前服务器的全 batch 基线，也兼容已切换完成的全 group 基线和原先的混合基线；混合占用同一通道或部分 group 运行时会拒绝操作。
...
```powershell
scp E:\river-video-app-V3.0\river-watch-ai-runtime-topology-increment-20260715-v2.zip `
    E:\river-video-app-V3.0\river-watch-ai-runtime-topology-increment-20260715-v2.zip.sha256 `
...
set -e
cd /home/ai-river
sha256sum -c river-watch-ai-runtime-topology-increment-20260715-v2.zip.sha256
rm -rf river-watch-ai-runtime-topology-increment-20260715-v2
unzip -o river-watch-ai-runtime-topology-increment-20260715-v2.zip
cd river-watch-ai-runtime-topology-increment-20260715-v2
sha256sum -c SHA256SUMS
bash -n scripts/*.sh
chmod +x scripts/*.sh
sudo -v

LOG="/home/ai-river/ai-runtime-group-standard-v2-apply-$(date +%Y%m%d-%H%M%S).log"
nohup sudo -n env \
...
```bash
cd /home/ai-river/river-watch-ai-runtime-topology-increment-20260715-v2
...
sudo bash scripts/rollback-ai-runtime-topology-20260714.sh \
  /home/ai-river/river-watch/backups/ai-runtime-group-standard-v2-20260715-实际时间戳
```
